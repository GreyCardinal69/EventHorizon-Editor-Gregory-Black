...
namespace GameDatabase.ShipLayout
{
    public class LayoutModel
    {
        ...
        public void DrawRectangle(int cell1, int cell2, char value)
        {
            ...
            int x1 = cell1 % _layout.Size;
            int y1 = cell1 / _layout.Size;
            int x2 = cell2 % _layout.Size;
            int y2 = cell2 / _layout.Size;
        ...
        public void DrawEllipse(int cell1, int cell2, char value)
        {
            ...
            float d = (x1 + x2) / 2f;
            float d2 = (y1 + y2) / 2f;
            float num = ((x2 - x1) / 2f) + 0.5f;
            float num2 = ((y2 - y1) / 2f) + 0.5f;
            for (int i = x1; i <= x2; i++)
                for (int j = y1; j <= y2; j++)
        ...
        public void DrawLine(int cell1, int cell2, char value)
        {
            ...
            {
                int x = x1 + ((x2 - x1) * ((2 * i) + 1) / (length * 2));
                int y = y1 + ((y2 - y1) * ((2 * i) + 1) / (length * 2));
                SetValue(x, y, value);
            }
