...
namespace GameDatabase.ShipLayout
{
    ...
    public abstract class GeometryBrush : ILayoutBrush
    {
        public virtual void OnDraw(PaintData paintData)
        {
            ...
            int[] mirrorCells1 = paintData.GetMirrorCells(StartPosition.CellIndex).ToArray();
            int[] mirrorCells2 = paintData.GetMirrorCells(CurrentPosition.CellIndex).ToArray();
            if (mirrorCells1.Length != mirrorCells2.Length)
                throw new InvalidOperationException();

            for (int i = 0; i < mirrorCells1.Length; ++i)
    ...
    public class RectangleBrush : GeometryBrush
    {
        ...
        protected override void DoDraw(PaintData data, int cell1, int cell2)
        {
            ...
            Point p1 = data.Align(x1 + (y1 * data.LayoutSize), PaintData.AlignSide.TopLeft);
            Point p2 = data.Align(x2 + (y2 * data.LayoutSize), PaintData.AlignSide.BottomRight);
    ...
    public class LineBrush : GeometryBrush
    {
        ...
        protected override void DoDraw(PaintData data, int cell1, int cell2)
        {
            Point p1 = data.Align(cell1, PaintData.AlignSide.Center);
            Point p2 = data.Align(cell2, PaintData.AlignSide.Center);
            Pen pen = new Pen(Color.Red, 3f);
