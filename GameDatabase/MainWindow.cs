    ...
    public partial class MainWindow : Form
    {
        ...
        private void OpenDatabase(string path)
        {
            ...
            try
            {
                ...
                DatabaseStorage storage = new DatabaseStorage(path);
                storage.LoadDatabaseVersion();
                int result = storage.Version.Compare(Database.VersionMajor, Database.VersionMinor);
                if (result > 0)
                ...
                if (result < 0)
                {
                    DialogResult dialogResult = MessageBox.Show($"Database version is outdated - {storage.Version}. Do you want to upgrade it to {Database.VersionMajor}.{Database.VersionMinor}?",
        ...
        private static Database UpgradeDatabase(IDataStorage storage, string path)
        {
            ...
            Database database = Database.MigrateFrom(storage);
        ...
        private void BuildFilesTree(string path, TreeNodeCollection nodeCollection)
        {
            try
            {
                ...
                {
                    string name = directory.Substring(directory.LastIndexOf("\\", StringComparison.OrdinalIgnoreCase) + 1);
                    TreeNode node = nodeCollection.Add(directory, name);
                    node.Tag = "Folder";
        ...
        private void BuildTemplates(string _path)
        {
            ...
            foreach (SerializableTemplate template in _templates.Data)
            {
                ...
                {
                    string[] path = template.Name.Split('/');
                    name = path.Last();
                    for (int i = 0; i < path.Length - 1; i++)
                    {
                        ToolStripItem[] findResult = lastNode.DropDownItems.Find(path[i], false);
                        if (findResult.Length == 0 || !(findResult[0] is AdvancedToolStripMenuItem curNode))
                        {
        ...
        private readonly Dictionary<string, string> ghostFiles = new Dictionary<string, string>();
        ...
        private void ShowItemInfo(string path)
        {
            ...
            string name = Helpers.FileName(path);
            SerializableItem item = JsonConvert.DeserializeObject<SerializableItem>(data);
            item.FileName = name;
        ...
        private DirectoryInfoData GetDirectoryInfo(string path)
        {
            ...
            try
            {
                ...
                foreach (string file in Directory.EnumerateFiles(path, "*.json", SearchOption.TopDirectoryOnly))
                {
                    ...
                    SerializableItem item = JsonConvert.DeserializeObject<SerializableItem>(text);
        ...
        private void EditButton_Click(object sender, EventArgs e)
        {
            ...
            string name = _selectedItem.FileName;
        ...
        private void MainWindow_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                DialogResult result = MessageBox.Show("Do you want to save database before closing?", "About to exit program?",
        ...
        public void createModMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!ModBuilder.TryReadSignature(_lastDatabasePath, out string name, out string guid))
                {
                    NameForm dialog = new NameForm();
                ...
                _database.Save(new DatabaseStorage(_lastDatabasePath));
                ModBuilder builder = ModBuilder.Create(_lastDatabasePath,
        ...
        {
            TreeNode name2 = DatabaseTreeView.SelectedNode;
        ...
        {
            string name = Prompt.ShowDialog("Input folder name", "");
            if (name == "") return;
            ...
            string _path = Path.Combine(_node.Name, name);
            ...
            TreeNode _folder = new TreeNode(name);
            _folder.Name = _path;
            _folder.Tag = "Folder";
        ...
        private void ItemFromTemplate(object sender, EventArgs e, string name, string __id)
        {
            ...
            {
                SerializableTemplate template = _templates.Get((sender as ToolStripMenuItem).Tag.ToString());
                ...
                string result = Prompt.ShowDialog(template.IdDialog, "", __id);
                if (result == "") return;
                int id;
                try
                {
                ...
                try
                {
                    SerializableTemplateItem[] items = template.Items;
                        ...
                        switch (field.Type)
                        {
                            case JTokenType.String:
                                ...
                                if (value.StartsWith("%n"))
                                {
                                    ...
                                    int num = FormatIntegerField(value);
                                    field.Replace(num);
                                }
                                else if (value.StartsWith("%s"))
                                {
                                    value = value.Replace("%s", "");
                                    string str = FormatStringField(value);
                                    field.Replace(str);
                                }
                                break;
                            ...
                            case JTokenType.Array:
                                foreach (JToken element in field.Value<JArray>())
                                {
                                    FormatField(element);
                                }
                                break;
                    ...
                    void BuildTemplateItems(SerializableTemplateItem[] _items, TreeNode _parentNode)
                    {
                        ...
                        {
                            SerializableTemplateItem item = _items[i];
                            string path = Path.Combine(_parentNode.Name, FormatStringField(item.Filename));
                            TreeNode _newNode;
                            if (item.Type == "Folder")
                            {
                            ...
                            nodes.Add(_newNode, _parentNode);
                            _files.Add(path);
                            JObject content = files[path] = (JObject)_items[i].Content.DeepClone();
                            FormatRecursive(content);

                            int _id = (int)content.GetValue("Id");
                            ...
                            ItemType type = (ItemType)(int)content.GetValue("ItemType");
                            /*        try
                                    {
        ...
        {
            foreach (string key in new List<string>(OppenedWindows.Keys))
            {
