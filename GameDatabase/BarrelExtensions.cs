...
namespace GameDatabase
{
    internal class BarrelExtensions
    {
        internal static Barrel[] CreateBarrels(string input)
        {
            ...
            int q = 0;
            foreach ((double, double) coordinate in coordinates)
            {
        ...
        private static void AddCoordinate(int row, int col, int n, int[,] grid, List<(double, double)> coordinates)
        {
            ...
            foreach ((double cx, double cy) in coordinates)
            {
            ...
            foreach ((int ox, int oy) in overlappingCoords)
            {
                coords.Add((oy, ox));
            }

            (double, double) v = GetCenter(coords.ToArray(), n);
