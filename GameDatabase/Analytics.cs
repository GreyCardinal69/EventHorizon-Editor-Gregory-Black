...
namespace GameDatabase
{
    public partial class Analytics : Form
    {
        ...
        private void RunTextAnalytics()
        {
            ...
            using (Hunspell hunspell = new Hunspell("en_us.aff", "en_us.dic"))
            {
                foreach (QuestSerializable quest in _database.Content.QuestList)
                {
                    if (quest.Nodes != null)
                    {
                        foreach (NodeSerializable node in quest.Nodes)
                        {
                            if (node.Type == NodeType.ShowDialog && node.Message[0] != '$')
                            {
                                ...
                                foreach (string word in node.Message.Split(' '))
                                {
            ...
            foreach (ComponentSerializable component in _database.Content.ComponentList)
            {
                ...
                if (component.Icon != null)
                {
                    if (!_database.Content.ImagesB.ContainsKey(component.Icon) && !vanillaImages.Contains(component.Icon))
                    {
                        ...
                        foreach (string name in _database.Content.ImagesB.Keys)
                        {
                            double distance = CalculateSimilarity(name.Replace(".png", ""), component.Icon.Replace(".png", "")) * 100;
                            if (distance >= 75)
                            {
                                Data.AppendText($"\n          Current Icon ID: {component.Icon}, did you mean: {name}?", Color.HotPink);
                            }
                        }
                        foreach (string name in vanillaImages)
                        {
                            double distance = CalculateSimilarity(name.Replace(".png", ""), component.Icon.Replace(".png", "")) * 100;
                            if (distance >= 75)
                            {
                ...
                if (component.CellType != null)
                {
                    if (!FormatValid(component.CellType))
                    {
                        ...
                        string anomaly = component.CellType
                            .Replace("1", "")
                            .Replace("2", "")
                            .Replace("3", "")
                            .Replace("4", "")
        ...
        private void RunOtherAnalytics()
        {
            ...
            foreach (AmmunitionSerializable ammo in _database.Content.AmmunitionList)
            {
            ...
            foreach (ShipSerializable item in _database.Content.ShipList)
            {
                shipIds.Add(item.Id);
            }
            ...
            foreach (FactionSerializable item in _database.Content.FactionList)
            {
            ...
            foreach (ShipBuildSerializable item in _database.Content.ShipBuildList)
            {
            ...
            foreach (int build in _database.Content.GalaxySettings.StartingShipBuilds)
            {
        ...
        private void RunShipAnalytics()
        {
            ...
            foreach (DeviceSerializable item in _database.Content.DeviceList)
            {
                devices.Add(item.Id);
            }

            foreach (ShipSerializable ship in _database.Content.ShipList)
            {
                ...
                {
                    foreach (int item in ship.BuiltinDevices)
                    {
        ...
        private void RunShipBuildAnalytics()
        {
            ...
            foreach (ComponentSerializable comp in _database.Content.ComponentList)
            {
                componentIds.Add(comp.Id);
            }

            foreach (ShipBuildSerializable build in _database.Content.ShipBuildList)
            {
                if (build.Components != null)
                {
                    foreach (InstalledComponentSerializable item in build.Components)
                    {
        ...
        private void RunLootAnalytics()
        {
            ...
            foreach (QuestItemSerializable questItem in _database.Content.QuestItemList)
            {
        ...
        private void RunTechAnalytics()
        {
            ...
            foreach (ComponentSerializable item in _database.Content.ComponentList)
            {
                componentIds.Add(item.Id);
            }
            foreach (SatelliteSerializable item in _database.Content.SatelliteList)
            {
            ...
            foreach (TechnologySerializable item in _database.Content.TechnologyList)
            {
            ...
            foreach (TechnologySerializable item in _database.Content.TechnologyList)
            {
                foreach (TechnologySerializable item2 in _database.Content.TechnologyList)
                {
            ...
            foreach (TechnologySerializable tech in _database.Content.TechnologyList)
            {
                switch (tech.Type)
                {
                ...
                if (tech.Dependencies != null)
                {
                    ...
                    {
                        foreach (int dep in tech.Dependencies)
                        {
        ...
        private void RunFleetAnalytics()
        {
            ...
            foreach (FleetSerializable fleet in _database.Content.FleetList)
            {
                ...
                foreach (int build in fleet.SpecificShips)
                {
                    builds.Add(build);
                }
            }
            ...
            foreach (FleetSerializable fleet in _database.Content.FleetList)
            {
                ...
                if (fleet.SpecificShips != null)
                {
                    foreach (int item in fleet.SpecificShips)
                    {
        ...
        private void RunQuestAnalytics()
        {
            ...
            foreach (QuestSerializable quest in _database.Content.QuestList)
            {
                ...
                foreach (NodeSerializable node in quest.Nodes)
                {
                    ...
                    {
                        foreach (NodeActionSerializable trans in node.Actions)
                        {
                    ...
                    {
                        foreach (NodeTransitionSerializable trans in node.Transitions)
                        {
                            if (!nodeIds.Contains(trans.TargetNode))
                            {
                                errorDetected = true;
        ...
        private void Statistics_Click(object sender, EventArgs e)
        {
            ...
            foreach (QuestSerializable node in _database.Content.QuestList)
            {
                if (node.Nodes != null)
                {
                    foreach (NodeSerializable item in node.Nodes)
                    {
