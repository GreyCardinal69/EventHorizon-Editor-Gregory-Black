...
namespace DatabaseMigration.v1
{
    public partial class DatabaseUpgrader
    {
        partial void Migrate_1_2()
        {
            ...
            int lootId = 0;
            foreach (LootSerializable item in Content.LootList)
                if (item.Id > lootId)
                    lootId = item.Id;
            lootId++;

            List<LootItemSerializable> itemList = new List<LootItemSerializable>();
            ...
            LootSerializable loot = new LootSerializable() { Id = lootId, FileName = "StartingInventory.json" };
            ...
            where T : SerializableItem
        {
            ...
            foreach ((int id, int amount) item in items)
