...
namespace DatabaseMigration.v1
{
    public partial class DatabaseUpgrader
    {
        ...
        {
            foreach (ShipBuildSerializable build in Content.ShipBuildList)
                if (build.BuildFaction < 0) build.BuildFaction = 0;
        }

        private void UpdateShips()
        {
            foreach (ShipSerializable ship in Content.ShipList)
            {
                if (ship.EngineSize > 0)
                {
                    EngineSerializable engine = new EngineSerializable { Position = ship.EnginePosition, Size = ship.EngineSize };
                    if (ship.Engines == null)
                        ship.Engines = new EngineSerializable[] { engine };
                    else
                    {
                        int length = ship.Engines.Length;
        ...
        {
            foreach (FactionSerializable faction in Content.FactionList)
            {
                if (faction.Hidden)
                {
        ...
        {
            foreach (ComponentStatsSerializable stats in Content.ComponentStatsList)
            {
        ...
        private void UpdateBarrels()
        {
            ...
            foreach (SatelliteSerializable satellite in Content.SatelliteList)
