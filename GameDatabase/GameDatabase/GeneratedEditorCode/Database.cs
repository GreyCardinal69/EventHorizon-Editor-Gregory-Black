...
namespace EditorDatabase
{
    ...
    public partial class Database
    {
        ...
        {
            Database secondaryDb = MainWindow.MainInstance._secondaryDatabse;

            foreach (KeyValuePair<int, AmmunitionObsolete> item in _ammunitionObsoleteMap)
            {
                AmmunitionObsoleteSerializable value = _content.GetAmmunitionObsolete(item.Key);
                if (value != secondaryDb.Content.GetAmmunitionObsolete(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, BehaviorTreeModel> item in _behaviorTreeMap)
            {
                BehaviorTreeSerializable value = _content.GetBehaviorTree(item.Key);
                if (value != secondaryDb.Content.GetBehaviorTree(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, GameObjectPrefab> item in _gameObjectPrefabMap)
            {
                GameObjectPrefabSerializable value = _content.GetGameObjectPrefab(item.Key);
                if (value != secondaryDb.Content.GetGameObjectPrefab(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, CombatRules> item in _combatRulesMap)
            {
                CombatRulesSerializable value = _content.GetCombatRules(item.Key);
                if (value != secondaryDb.Content.GetCombatRules(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Component> item in _componentMap)
            {
                ComponentSerializable value = _content.GetComponent(item.Key);
                if (value != secondaryDb.Content.GetComponent(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, ComponentMod> item in _componentModMap)
            {
                ComponentModSerializable value = _content.GetComponentMod(item.Key);
                if (value != secondaryDb.Content.GetComponentMod(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, ComponentStats> item in _componentStatsMap)
            {
                ComponentStatsSerializable value = _content.GetComponentStats(item.Key);
                if (value != secondaryDb.Content.GetComponentStats(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Device> item in _deviceMap)
            {
                DeviceSerializable value = _content.GetDevice(item.Key);
                if (value != secondaryDb.Content.GetDevice(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, DroneBay> item in _droneBayMap)
            {
                DroneBaySerializable value = _content.GetDroneBay(item.Key);
                if (value != secondaryDb.Content.GetDroneBay(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Faction> item in _factionMap)
            {
                FactionSerializable value = _content.GetFaction(item.Key);
                if (value != secondaryDb.Content.GetFaction(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Satellite> item in _satelliteMap)
            {
                SatelliteSerializable value = _content.GetSatellite(item.Key);
                if (value != secondaryDb.Content.GetSatellite(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, SatelliteBuild> item in _satelliteBuildMap)
            {
                SatelliteBuildSerializable value = _content.GetSatelliteBuild(item.Key);
                if (value != secondaryDb.Content.GetSatelliteBuild(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Ship> item in _shipMap)
            {
                ShipSerializable value = _content.GetShip(item.Key);
                if (value != secondaryDb.Content.GetShip(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, ShipBuild> item in _shipBuildMap)
            {
                ShipBuildSerializable value = _content.GetShipBuild(item.Key);
                if (value != secondaryDb.Content.GetShipBuild(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, StatUpgradeTemplate> item in _statUpgradeTemplateMap)
            {
                StatUpgradeTemplateSerializable value = _content.GetStatUpgradeTemplate(item.Key);
                if (value != secondaryDb.Content.GetStatUpgradeTemplate(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Technology> item in _technologyMap)
            {
                TechnologySerializable value = _content.GetTechnology(item.Key);
                if (value != secondaryDb.Content.GetTechnology(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Character> item in _characterMap)
            {
                CharacterSerializable value = _content.GetCharacter(item.Key);
                if (value != secondaryDb.Content.GetCharacter(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Fleet> item in _fleetMap)
            {
                FleetSerializable value = _content.GetFleet(item.Key);
                if (value != secondaryDb.Content.GetFleet(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, LootModel> item in _lootMap)
            {
                LootSerializable value = _content.GetLoot(item.Key);
                if (value != secondaryDb.Content.GetLoot(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, QuestModel> item in _questMap)
            {
                QuestSerializable value = _content.GetQuest(item.Key);
                if (value != secondaryDb.Content.GetQuest(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, ComponentStatUpgrade> item in _componentStatUpgradeMap)
            {
                ComponentStatUpgradeSerializable value = _content.GetComponentStatUpgrade(item.Key);
                if (value != secondaryDb.Content.GetComponentStatUpgrade(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, QuestItem> item in _questItemMap)
            {
                QuestItemSerializable value = _content.GetQuestItem(item.Key);
                if (value != secondaryDb.Content.GetQuestItem(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Ammunition> item in _ammunitionMap)
            {
                AmmunitionSerializable value = _content.GetAmmunition(item.Key);
                if (value != secondaryDb.Content.GetAmmunition(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, BulletPrefab> item in _bulletPrefabMap)
            {
                BulletPrefabSerializable value = _content.GetBulletPrefab(item.Key);
                if (value != secondaryDb.Content.GetBulletPrefab(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, VisualEffect> item in _visualEffectMap)
            {
                VisualEffectSerializable value = _content.GetVisualEffect(item.Key);
                if (value != secondaryDb.Content.GetVisualEffect(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, Weapon> item in _weaponMap)
            {
                WeaponSerializable value = _content.GetWeapon(item.Key);
                if (value != secondaryDb.Content.GetWeapon(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, ComponentGroupTag> item in _componentGroupTagMap)
            {
                ComponentGroupTagSerializable value = _content.GetComponentGroupTag(item.Key);
                if (value != secondaryDb.Content.GetComponentGroupTag(item.Key))
                    item.Value.Save(value);
            }

            foreach (KeyValuePair<int, WeaponSlots> item in _weaponSlotsMap)
            {
                WeaponSlotsSerializable value = _content.GetWeaponSlots(item.Key);
        ...
        public void SetItem(ItemType type, int id, int old)
        {
            switch (type)
            {
                ...
                case ItemType.StatUpgradeTemplate: SetStatUpgradeTemplate(id, old); break;
        ...
        {
            if (!_weaponSlotsMap.TryGetValue(id, out WeaponSlots item))
            {
        ...
        {
            if (!_componentStatUpgradeMap.TryGetValue(id, out ComponentStatUpgrade item))
            {
        ...
        {
            if (!_ammunitionObsoleteMap.TryGetValue(id, out AmmunitionObsolete item))
            {
        ...
        {
            if (!_behaviorTreeMap.TryGetValue(id, out BehaviorTreeModel item))
            {
        ...
        {
            if (!_combatRulesMap.TryGetValue(id, out CombatRules item))
            {
        ...
        {
            if (!_gameObjectPrefabMap.TryGetValue(id, out GameObjectPrefab item))
            {
                GameObjectPrefabSerializable serializable = _content.GetGameObjectPrefab(id);
        ...
        {
            if (!_componentMap.TryGetValue(id, out Component item))
            {
                ComponentSerializable serializable = _content.GetComponent(id);
        ...
        {
            if (!_componentModMap.TryGetValue(id, out ComponentMod item))
            {
                ComponentModSerializable serializable = _content.GetComponentMod(id);
        ...
        {
            if (!_componentStatsMap.TryGetValue(id, out ComponentStats item))
            {
                ComponentStatsSerializable serializable = _content.GetComponentStats(id);
        ...
        {
            if (!_deviceMap.TryGetValue(id, out Device item))
            {
                DeviceSerializable serializable = _content.GetDevice(id);
        ...
        {
            if (!_droneBayMap.TryGetValue(id, out DroneBay item))
            {
                DroneBaySerializable serializable = _content.GetDroneBay(id);
        ...
        {
            if (!_factionMap.TryGetValue(id, out Faction item))
            {
                FactionSerializable serializable = _content.GetFaction(id);
        ...
        {
            if (!_satelliteMap.TryGetValue(id, out Satellite item))
            {
                SatelliteSerializable serializable = _content.GetSatellite(id);
        ...
        {
            if (!_satelliteBuildMap.TryGetValue(id, out SatelliteBuild item))
            {
                SatelliteBuildSerializable serializable = _content.GetSatelliteBuild(id);
        ...
        {
            if (!_shipMap.TryGetValue(id, out Ship item))
            {
                ShipSerializable serializable = _content.GetShip(id);
        ...
        {
            if (!_shipBuildMap.TryGetValue(id, out ShipBuild item))
            {
                ShipBuildSerializable serializable = _content.GetShipBuild(id);
        ...
        {
            if (!_statUpgradeTemplateMap.TryGetValue(id, out StatUpgradeTemplate item))
            {
                StatUpgradeTemplateSerializable serializable = _content.GetStatUpgradeTemplate(id);
        ...
        {
            if (!_technologyMap.TryGetValue(id, out Technology item))
            {
                TechnologySerializable serializable = _content.GetTechnology(id);
        ...
        {
            if (!_characterMap.TryGetValue(id, out Character item))
            {
                CharacterSerializable serializable = _content.GetCharacter(id);
        ...
        {
            if (!_fleetMap.TryGetValue(id, out Fleet item))
            {
                FleetSerializable serializable = _content.GetFleet(id);
        ...
        {
            if (!_lootMap.TryGetValue(id, out LootModel item))
            {
                LootSerializable serializable = _content.GetLoot(id);
        ...
        {
            if (!_questMap.TryGetValue(id, out QuestModel item))
            {
                QuestSerializable serializable = _content.GetQuest(id);
        ...
        {
            if (!_questItemMap.TryGetValue(id, out QuestItem item))
            {
                QuestItemSerializable serializable = _content.GetQuestItem(id);
        ...
        {
            if (!_ammunitionMap.TryGetValue(id, out Ammunition item))
            {
                AmmunitionSerializable serializable = _content.GetAmmunition(id);
        ...
        {
            if (!_bulletPrefabMap.TryGetValue(id, out BulletPrefab item))
            {
                BulletPrefabSerializable serializable = _content.GetBulletPrefab(id);
        ...
        {
            if (!_visualEffectMap.TryGetValue(id, out VisualEffect item))
            {
                VisualEffectSerializable serializable = _content.GetVisualEffect(id);
        ...
        {
            if (!_weaponMap.TryGetValue(id, out Weapon item))
            {
                WeaponSerializable serializable = _content.GetWeapon(id);
        ...
        {
            ComponentStatUpgradeSerializable serializable = _content.GetComponentStatUpgrade(id);
            _content.ComponentStatUpgradeList[_content.ComponentStatUpgradeList.IndexOf(_content.GetComponentStatUpgrade(old))] = serializable;
            _componentStatUpgradeMap[old] = new ComponentStatUpgrade(serializable, this);
        ...
        public void SetStatUpgradeTemplate(int id, int old)
        {
            StatUpgradeTemplateSerializable serializable = _content.GetStatUpgradeTemplate(id);
            _content.StatUpgradeTemplateList[_content.StatUpgradeTemplateList.IndexOf(_content.GetStatUpgradeTemplate(old))] = serializable;
            _statUpgradeTemplateMap[old] = new StatUpgradeTemplate(serializable, this);
        }
        ...
        {
            ComponentGroupTagSerializable serializable = _content.GetComponentGroupTag(id);
